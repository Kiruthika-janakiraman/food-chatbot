...
for intent in intents['intents']:
    for pattern in intent['patterns']:
        ...
        responses.append(intent['response'])  # Use 'response' instead of 'responses'
...
def get_response(tag):
    for intent in intents['intents']:
        if intent['tag'] == tag:
            return random.choice(intent['response'])  # Use 'response' instead of 'responses'

# Streamlit interface
st.title("Food Chatbot")
st.write("Ask me anything about food, nutrition, and recipes!")
